#!/bin/bash
# ai-service 빌드 & Docker Hub 푸시 + 서버 배포 자동화 스크립트
...
IMAGE="gurururu/ai-service:latest"
...
ssh -i $SSH_KEY -o StrictHostKeyChecking=no $SERVER "
  cd /home/ubuntu && \
  sudo docker stop ai-service 2>/dev/null || true && \
  sudo docker rm -f ai-service 2>/dev/null || true && \
  sudo docker compose -f ai-compose.yml up -d
"

echo "✅ 배포 완료! http://$SERVER_IP:8083"
